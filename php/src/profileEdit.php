...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt_email = $conn->prepare("SELECT COUNT(*) FROM users WHERE user_email = ? AND user_id != ?");
    if (!$stmt_email) {
        echo "<script>
    ...
    $stmt_email->bind_param("ss", $user_email, $user_id); // $user_id คือ ID ของผู้ใช้ปัจจุบัน
    $stmt_email->execute();
    ...
    $stmt_phone = $conn->prepare("SELECT COUNT(*) FROM users WHERE user_phone = ? AND user_id != ?");
    if (!$stmt_phone) {
        echo "<script>
    ...
    $stmt_phone->bind_param("ss", $user_phone, $user_id); // $user_id คือ ID ของผู้ใช้ปัจจุบัน
    $stmt_phone->execute();
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="subdistrict"
                        class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                        id="district" value="<?php echo htmlspecialchars($user_data['subdistrict']); ?>" required>
                </div>
                <div>
                    <label class="block text-gray-700 text-sm font-bold mb-2">District</label>
                    <input type="text" name="district"
                        class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                        id="amphoe" value="<?php echo htmlspecialchars($user_data['district']); ?>" required>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="province"
                        class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                        id="province" value="<?php echo htmlspecialchars($user_data['province'] ?? ''); ?>" required>
                </div>
                <div>
                    <label class="block text-gray-700 text-sm font-bold mb-2">Postal Code</label>
                    <input type="text" name="postal_code"
                        class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                        id="zipcode" value="<?php echo htmlspecialchars($user_data['postal_code'] ?? ''); ?>" required>
